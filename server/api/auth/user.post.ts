...
export default eventHandler(async (event) => {
    ...
    if (fields["username"] && fields["username"][0] && user.username !== fields["username"][0]) {
        result["username"] = fields["username"][0]
    }
    if (fields["tel"] && fields["tel"][0] && user.tel !== fields["tel"][0]) {
        result["tel"] = fields["tel"][0]
    }
    if (fields["nickname"] && fields["nickname"][0] && user.nickname !== fields["nickname"][0]) {
    ...
    if (files && Object.keys(files).length) {
        const { file: [{ filepath, mimetype, originalFilename }] } = files
        if (originalFilename.endsWith(".png") || originalFilename.endsWith(".jpg") || originalFilename.endsWith(".jpeg")) {
            ...
            const p = newPath.replace(/^public/, "")
            result["avatar"] = p
        }
    }
    try {
